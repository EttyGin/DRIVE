...
@app.post("/internal/api-keys/{user_id}")
def update_api_key(
    user_id: int,
    payload: ApiKeyUpdateRequest,
    acting_user=Depends(authenticate_request),
):
    if not isinstance(acting_user, LocalUser):
        raise HTTPException(status_code=403, detail="Local admin only")

    set_api_key_for_user(
        acting_user=acting_user,
        target_user_id=user_id,
        new_raw_api_key=payload.api_key,
    )
...
def bootstrap():
    ...
    admin.api_key_hash = hash_api_key("ADMIN_SUPER_SECRET_KEY_123456")
    regular.api_key_hash = hash_api_key("REGULAR_SERVICE_KEY_abcdef")
